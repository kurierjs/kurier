...
describe("Articles", () => {
  describe("GET", () => {
    beforeEach(async () => {
      const article = deserializer(articles.toGet.initial[0], Article, "add");
    ...
    it("Get all articles", async () => {
      ...
      expect(result.body).toEqual({ data: articles.toGet.loaded });
    ...
    it("Gets article by id", async () => {
      ...
      expect(result.status).toEqual(200);
      expect(result.body).toEqual({ data: articles.toGet.loaded[0] });
    });
  });
});
